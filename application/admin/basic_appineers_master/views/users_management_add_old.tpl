...
<div class="form-row row-fluid " id="cc_sh_u_dob">
    ...
    <div class="form-row row-fluid " id="cc_sh_u_city">
        ...
        <div class="form-right-div  <%if $mode eq 'Update'%>frm-elements-div<%/if%> ">
            <%if $mode eq "Update"%>
                <input type="hidden" class="ignore-valid" name="u_state_name" id="u_state_name" value="<%$data['u_state_name']|@htmlentities%>" />
                <span class="frm-data-label">
                    <strong>
                        <%if $data['u_state_name'] neq ""%>
                            <%$data['u_state_name']%>
                        <%else%>
                        <%/if%>
                    </strong></span>
                <%else%>
                    <input type="text" placeholder="" value="<%$data['u_city']|@htmlentities%>" name="u_state_name" id="u_state_name" title="<%$this->lang->line('USERS_MANAGEMENT_STATE')%>"  data-ctrl-type='textbox'  class='frm-size-medium'  />
                <%/if%>
            </div>
